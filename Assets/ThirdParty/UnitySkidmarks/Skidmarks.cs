...
public class Skidmarks : MonoBehaviour 
{
	...
	[SerializeField] private Material _material; // Material for the skidmarks to use	

	// END INSPECTOR SETTINGS	
	private WheelSkid _wheelSkid;

    const int MAX_MARKS = 64; // Max number of marks total for everyone together
    ...
    public void Init(WheelSkid wheelSkid)
    {		
		_wheelSkid = wheelSkid;
    }
    ...
    // Function called by the wheel that's skidding. Sets the intensity of the skidmark section, in the default _color.
    public int AddSkidMark(Vector3 pos, Vector3 normal, float opacity, int lastIndex, float mark_width) 
	{
		if (opacity > 1) 
			opacity = 1.0f;
		else 
        ...
        if (_wheelSkid.GroundMaterial == GroundMaterial.Blocker)
            return -1;

        _color.a = (byte)(opacity * 255);
		return AddSkidMark(pos, normal, _color, lastIndex, mark_width);
	}

	// Function called by the wheel that's skidding. Sets the colour and intensity of the skidmark section.
	public int AddSkidMark(Vector3 pos, Vector3 normal, Color32 colour, int lastIndex, float mark_width) 
	{
		if (colour.a == 0) 
