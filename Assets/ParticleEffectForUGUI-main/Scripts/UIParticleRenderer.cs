...
namespace Coffee.UIExtensions
{
    ...
    internal class UIParticleRenderer : MaskableGraphic
    {
        ...
        public void UpdateMesh(Camera bakeCamera)
        {
            ...
            {
                _renderer.BakeTrailsMesh(s_CombineInstances[0].mesh, bakeCamera, ParticleSystemBakeMeshOptions.Default);                
            }
            else if (_renderer.CanBakeMesh())
            {
                _renderer.BakeMesh(s_CombineInstances[0].mesh, bakeCamera, ParticleSystemBakeMeshOptions.Default);
            }
            else
            {
